...
{
    RG_LOGI("Using ES8311 DAC");

    i2s_driver_uninstall(I2S_NUM_0);
    gpio_reset_pin(RG_GPIO_SND_I2S_MCK);
    ...
    state.device = device;
        ...
        if (ret == ESP_OK)
        {
            ...
            RG_LOGI("I2S Data out: %d",RG_GPIO_SND_I2S_DATA);
        }
        if (ret != ESP_OK)
...
uint8_t dummy[4] = {0};
size_t written;
i2s_write(I2S_NUM_0, dummy, sizeof(dummy), &written, portMAX_DELAY);
rg_task_delay(100);
if(es8311_codec_init() != ESP_OK)
    {
        RG_LOGE("Error initializing ES8311");
    }
